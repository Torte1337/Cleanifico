...
| --- | --- | --- |
| `Cleanifico.Domain` | Entities, Value Objects, Enums, Domain-Regeln und Domain Exceptions | `CleaningType`, `TimeType`, `Customer`, `CleaningObject`, `Employee` und `EmployeeContract` mit UTC-Lifecycle |
| `Cleanifico.Application` | Use Cases, Interfaces, Validierung und Orchestrierung | Reinigungstyp-, Zeittyp-, Kunden-, Objekt-, Mitarbeiter-, Vertrags- und Benutzerverwaltungs-Ports |
...
`CleanificoDbContext` ist der zentrale Context. Entity-Mappings liegen in separaten `IEntityTypeConfiguration<T>`-Klassen und werden aus der Infrastructure-Assembly geladen. `CleaningType` und `TimeType` besitzen eindeutige, case-insensitive Indizes für Name und Code. `Customer`, `CleaningObject`, `Employee` und `EmployeeContract` besitzen eindeutige tenantlokale Geschäftsnummern. CleaningObject zu Customer und EmployeeContract zu Employee verwenden verpflichtende `Restrict`-Fremdschlüssel. Lookup-Indizes unterstützen Status-, Zeitraum-, Zuordnungs- und Ortssuchen. Technische Zeitstempel werden als `datetime(6)` in UTC geführt.
...
Schemaänderungen liegen versioniert unter `Persistence/Migrations`. Auf `InitialCleanificoPersistence`, `AddTenantIdentity`, `AddConfigurableTimeTypes`, `AddCustomers`, `AddCleaningObjects` und `AddEmployees` folgt `AddEmployeeContracts`. Diese Migration übernimmt vorhandene Beschäftigungswerte in historische Vertragsdatensätze, bevor die alten Employee-Spalten entfallen. Der Host führt weder `EnsureCreated` noch `Database.Migrate` beim Start aus. Lokale und spätere produktive Tenant-Migrationen werden explizit und kontrolliert ausgeführt.
...
`Cleanifico.Api` ist der serverseitige Composition Root. Sie registriert Problem Details, zentrale Exception-Behandlung, Health Checks, Application Services und die Infrastructure-Adapter. Neben `GET /health` stellt sie Stammdaten-Routen unter `/api/cleaning-types`, `/api/time-types`, `/api/customers`, `/api/objects`, `/api/employees` und `/api/employee-contracts` bereit. Domainvalidierung führt zu `400`, fehlende Datensätze zu `404` und Eindeutigkeits-/Lösch-/Zeitraumkonflikte zu `409`; unerwartete Fehler werden geloggt, aber ohne interne Details ausgeliefert.

`Cleanifico.Web` ist eine eigenständige Blazor-Web-App mit serverseitiger Interaktivität. Sie referenziert nur öffentliche Contracts und ruft die API über typisierte HTTP-Clients auf. Die Seiten `/reinigungstypen`, `/zeittypen`, `/kunden`, `/objekte`, `/mitarbeiter` und `/mitarbeitervertraege` bieten Suche, Statusfilter, Listen, Dialoge zum Anlegen/Bearbeiten und klar getrennte Lifecycle-Aktionen. Objekt- und Vertragslisten filtern zusätzlich nach ihrer fachlichen Zuordnung; Mitarbeiterdetails zeigen aktuellen Vertrag, Vertragsanzahl und Historie mit Direktlinks. Backendfehler werden als sichere deutsche Meldungen dargestellt; rohe Antworttexte oder Stacktraces werden nicht gezeigt.
...
`EmployeeNumber` ist ein benutzerverwalteter, case-insensitive eindeutiger Geschäftsschlüssel. Employee führt nur persönliche Stamm-, Adress- und Kontaktdaten sowie seinen Lifecycle; Beschäftigungszeitraum, Beschäftigungsart und Stunden werden dort nicht mehr gepflegt.

`EmployeeContract` ist die fachliche Quelle für historienfähige Beschäftigungsbedingungen. Ein Employee besitzt null bis viele Verträge. `ContractNumber` ist tenantlokal case-insensitive eindeutig und änderbar; Beschäftigungsart bleibt optionaler frei pflegbarer Text statt Enum. Vertragsbeginn ist erforderlich, Ende und Probezeit unterliegen Datumsregeln, und Stunden sowie Urlaubstage sind nichtnegativ. Aktive Vertragszeiträume desselben Mitarbeiters dürfen sich nicht überschneiden; Folgeverträge bleiben als eigene Datensätze erhalten. Deaktivierung ändert den fachlichen Zeitraum nicht automatisch.

Der verpflichtende Employee-Fremdschlüssel verwendet `Restrict`. Ein Employee mit mindestens einem Vertrag kann weder im Application Service noch in MySQL physisch gelöscht werden. Ein Vertrag darf aktuell ohne weitere Referenzen physisch gelöscht werden; sobald Arbeitszeiten, Abrechnungen oder andere historische Vorgänge ihn referenzieren, müssen deren Restrict-Fremdschlüssel Löschen verhindern und Beenden/Deaktivieren wird regulär.
...
Die Rollen sind `Owner`, `Administrator`, `Dispatcher`, `ObjectManager` und `Employee`. Rollen und Policy-Namen liegen zentral in Contracts. Die API erzwingt zusätzlich zu den Rollen eine Active-User-Anforderung als Fallback-Policy. Fachliche Lesezugriffe einschließlich `ViewEmployees` und `ViewEmployeeContracts` erlauben die vier Office-Rollen, Schreibzugriffe einschließlich `ManageEmployees` und `ManageEmployeeContracts` nur Owner und Administrator. Benutzerverwaltung und Rollenvergabe sind ebenfalls Owner/Administrator vorbehalten. Der letzte aktive Owner ist gegen Deaktivierung und Rollenentzug geschützt.
...
API-Business-Policies für CleaningTypes, TimeTypes, Customers, CleaningObjects, Employees und EmployeeContracts enthalten zusätzlich `LicensedProductRequirement`. Anonyme Aufrufe bleiben `401`; bei gültiger Lizenz und fehlender Rolle bleibt `403`. Ungültige oder nicht prüfbare Lizenzen liefern ein kontrolliertes `403 ProblemDetails` ohne interne URL oder Exception. `/api/license/status` ist für aktive Benutzer lizenzunabhängig erreichbar. `/health`, Login, Logout, Sessionprüfung und Benutzeradministration werden nicht durch die Geschäftslizenz blockiert.

Cleanifico Office spiegelt dieselbe zusätzliche Policy-Anforderung für `/reinigungstypen`, `/zeittypen`, `/kunden`, `/objekte`, `/mitarbeiter` und `/mitarbeitervertraege`. Der Status wird über die authentifizierte Cleanifico API bezogen und pro Server-Circuit eine Minute gecacht. `/lizenz` zeigt Lease- und Installationsstatus; Owner/Administrator dürfen dort aktivieren und manuell erneuern. Rollenprüfungen bleiben vollständig bestehen.
