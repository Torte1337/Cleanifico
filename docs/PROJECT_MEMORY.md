...
- Die Migrationen heißen `InitialCleanificoPersistence`, `AddTenantIdentity` und `AddConfigurableTimeTypes`; sie werden nicht automatisch beim Hoststart ausgeführt.
...
- Rollen: `Owner`, `Administrator`, `Dispatcher`, `ObjectManager`, `Employee`. Zentrale Policies umfassen Office-Zugang, Lese-/Schreibrechte für CleaningType und TimeType, Benutzer-/Rollenverwaltung sowie die interne Aktivitätsprüfung.
...
- Cleanifico Office stellt `/login`, `/zugriff-verweigert`, `/reinigungstypen`, `/zeittypen` und `/administration/benutzer` bereit; eine öffentliche Registrierung existiert nicht.
